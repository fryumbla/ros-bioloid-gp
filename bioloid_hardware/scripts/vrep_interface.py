#!/usr/bin/env python
import roslib
roslib.load_manifest('bioloid_hardware')
import rospkg
...
class VrepInterface():
    ...
    def __init__(self):
        ...
        self.client_id = vrep.simxStart('127.0.0.1', 19997, True, True, 1000, 5)  # Connect to V-REP
        assert self.client_id != -1, 'Fail to connect the VREP!'

        if self.client_id != -1:
            vrep.simxSynchronous(self.client_id, True)
            vrep.simxSetFloatingParameter(self.client_id, vrep.sim_floatparam_simulation_time_step, self.dT, vrep.simx_opmode_blocking)
    ...
    def get_obstacles(self):
        ...
        for object_name in self.active_objects:
            ...
            _, position = vrep.simxGetObjectPosition(clientID=self.client_id,
                    objectHandle=h,
                    relativeToObjectHandle=-1,
                    operationMode=vrep.simx_opmode_buffer)
                    #operationMode=vrep.simx_opmode_blocking)

            _, quat = vrep.simxGetObjectQuaternion(clientID=self.client_id,
                    objectHandle=h,
                    relativeToObjectHandle=-1,
                    operationMode=vrep.simx_opmode_buffer)
                    #operationMode=vrep.simx_opmode_blocking)
    ...
    def get_joint_states(self):
        ...
        ret, handles, idata, fdata, sdata = vrep.simxGetObjectGroupData(clientID=self.client_id,
                        dataType=15,
        ...
        for joint_name in self.active_joints:
            ...
            # _, q = vrep.simxGetJointPosition(clientID=self.client_id,
            #         jointHandle=h,
            #         operationMode=vrep.simx_opmode_blocking)
            #         #operationMode=vrep.simx_opmode_buffer)
    ...
    def set_target_joint_positions(self, **joint_goal):
        ...
        for ii, jn in enumerate(joint_goal):
            if jn in self.active_joints:
                ...
                res = vrep.simxSetJointTargetPosition(clientID=self.client_id,
                        jointHandle=jh,
    ...
    def start_simulation(self):
        ...
        '''
        # start simulation
        res = vrep.simxStartSimulation(self.client_id, vrep.simx_opmode_oneshot)
        if res > 2:
        ...
        # for joint_name in self.active_joints:
        #     h = self.active_joints.get(joint_name)
        #     vrep.simxGetJointPosition(clientID=self.client_id,
        #             jointHandle=h,
        ...
        for object_name in self.active_objects:
            h = self.active_objects.get(object_name)
            vrep.simxGetObjectPosition(clientID=self.client_id,
                    objectHandle=h,
                    relativeToObjectHandle=-1,
                    operationMode=vrep.simx_opmode_streaming)

            vrep.simxGetObjectQuaternion(clientID=self.client_id,
                    objectHandle=h,
                    relativeToObjectHandle=-1,
                    operationMode=vrep.simx_opmode_streaming)
    ...
    def load_sim_model(self, model_path):
        ...
        if vrep.simxGetConnectionId(self.client_id) != -1:
            ...
            if vrep.simxLoadScene(self.client_id, str(model_path), 1, vrep.simx_opmode_blocking):
                print('VREP: scene load error')
                return -1

            # load model -> modified because of some problems
            #res, self.object_handle = vrep.simxLoadModel(self.clientID, str(object_path), 1, vrep.simx_opmode_blocking)
            #res, self.object_handle = vrep.simxGetObjectHandle(clientID=self.client_id,
    ...
    def get_joint_lists(self, cid):
        ...
        ret,handles,_,_,sdata = vrep.simxGetObjectGroupData(clientID=cid, 
                dataType=0,  
                objectType=vrep.sim_object_joint_type, 
                operationMode=vrep.simx_opmode_blocking)
        if ret != 0:
        ...
        ret,_,idata,_,_ = vrep.simxGetObjectGroupData(clientID=cid, 
                dataType=16,  
                objectType=vrep.sim_object_joint_type, 
                operationMode=vrep.simx_opmode_blocking)
        if ret != 0:
        ...
        for i,key in enumerate(all_joints):
            ret, mode = vrep.simxGetObjectIntParameter(clientID=cid,
                    objectHandle=all_joints[key],
                    parameterID=vrep.sim_jointintparam_motor_enabled,
                    operationMode=vrep.simx_opmode_blocking)
    ...
    def get_object_boundingbox(self, cid, **obstacles):
        ...
        for i,n in enumerate(obstacles):
            ...
            for j in range(6):
                res, bb[j] = vrep.simxGetObjectFloatParameter(clientID=cid, 
                            objectHandle=h, 
                            operationMode=vrep.simx_opmode_blocking, 
                            parameterID=15+j)
...
from bioloid_hardware.srv import *
import rosparam
import rospkg
...
class RosInterface:
    ...
    def callback_set_motion(self, req):
        ...
        #rospy.loginfo(req.trajectory)

        if self.action_state == RosInterface.ACTION_READY:
            res.result = VrepSetJointTrajectoryResponse.OK
            self.desired_trajectory['duration'] = req.duration
            self.desired_trajectory['trajectory'] = req.trajectory
            self.action_state = RosInterface.ACTION_BUSY
            self.action_start_time = 0
            self.current_time = 0
        else:
            res.result = VrepSetJointTrajectoryResponse.ERROR
    ...
    def update_goal(self):
        ...
        # check whether the action is activated or not.
        if self.action_state == RosInterface.ACTION_READY:
            return joint_goal

        # elapsed time from the action start
        #start_time = self.action_start_time
        #t_secs = (current_time-start_time).secs
        #t_nsecs = (current_time-start_time).nsecs
        #t_from_start = t_secs + t_nsecs/1e9
        t_from_start = self.current_time - self.action_start_time
...
if __name__ == '__main__':
    ...
    try:
        rp = rospkg.RosPack()
        conf_path = rp.get_path('bioloid_hardware') + '/config/jointremap.yaml'
    ...
    sim_model_path = '/home/francisco/robocare_ws/src/social_robot_description/vrep_model/social_robot_default.ttt'
    ...
    # ROS topic
    #pub_object_info = rospy.Publisher('~object_info', JointState, queue_size=10)
    ...
    while not rospy.is_shutdown():
        ...
        # write states
        #dT = 1/loop_freq
        joint_goal = ros_if.update_goal()
        vrep_if.set_target_joint_positions(**joint_goal)
