  ...
  as_(nh_, action_name_, false)
{
  ...
  // joint_command_pub=nh_.advertise<sensor_msgs::JointState>("/panda/joint_set",1);


  joint_command_msg.name.resize(20);
  joint_command_msg.position.resize(20);

  joint_command_msg.header.frame_id="";
  
  for(int i=0;i<21;i++){
  joint_command_msg.name[i] = "joint_" +std::to_string(i+1);
  }
  ...
  as_(nh_, action_name_, false)
{
  ...
  joint_command_pub=nh_.advertise<sensor_msgs::JointState>("/panda/joint_set",100);
